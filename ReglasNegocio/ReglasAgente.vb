...
Namespace ReglasAgente
    ...
    Public Class ReglasAgente
        ...
        Public Function VerificarColasPegasdas() As ResponseObject
            ...
            Try
                ReglasLog.ReglasLog.CrearLog("VerificaColasPegadas", "Inicio", ReglasLog.ReglasLog.TipoLog.Info, 1, 1)

                Dim _comando As New SqlClient.SqlCommand("ProgramacionCola.MostrarColasPegadas")
                _comando.CommandType = CommandType.StoredProcedure

                _conexion = New CapaDatosSql(ConfigurationManager.ConnectionStrings("SensoresTI").ToString, False)
            ...
            ReglasLog.ReglasLog.CrearLog("VerificaColasPegadas", "Fin", 1, 1, 1)
        ...
        Public Function VerificarProgramacionHOST() As ResponseObject
            ...
            Try
                ReglasLog.ReglasLog.CrearLog("VerificaProgramacion", "Inicio", ReglasLog.ReglasLog.TipoLog.Info, 1, 1)

                Dim _comando As New SqlClient.SqlCommand("CopiadoArchivos.IniciarColaProgramacionDisponible")
                _comando.CommandType = CommandType.StoredProcedure
                ...
                If Not _programacion Is Nothing Then
                    If _programacion.IdColaProgramacion > 0 Then
                        Try
                            ...
                            Select Case _programacion.TipoProgramacion
                                Case 2 ' copiar
                                    ...
                                    Select Case _programacion.TipoColaProgramacion
                                        Case TipoColaProgramacion.Validar
                                            ...
                                            If Not _resultado.IsError And _resultado.Objeto = True Then
                                                ...
                                                _comando = New SqlClient.SqlCommand("CopiadoArchivos.InsertarCopiaProgramada")
                                                _comando.CommandType = CommandType.StoredProcedure
                                        ...
                                        Case TipoColaProgramacion.CopiaProgramada
                                            ReglasLog.ReglasLog.CrearLog(_programacion.DescripcionProgramacion, "PUBLICACION PROGRAMADA | Inicio ", ReglasLog.ReglasLog.TipoLog.Info, _programacion.VerbosidadLog, 1)
                                ...
                                Case Else
                                    ReglasLog.ReglasLog.CrearLog(_programacion.DescripcionProgramacion, "Se encuentra programación sin tipo de conocido " & _programacion.IdColaProgramacion.ToString(),
                                                                  ReglasLog.ReglasLog.TipoLog.Err, _programacion.VerbosidadLog, 1)
                            ...
                            ' termina la cola
                            _comando = New SqlClient.SqlCommand("CopiadoArchivos.TerminarColaProgramacion")
                            _comando.CommandType = CommandType.StoredProcedure
                            _conexion = New CapaDatosSql(ConfigurationManager.ConnectionStrings("SensoresTI").ToString, False)
                    ...
                    Else
                        ReglasLog.ReglasLog.CrearLog("VerificaProgramacion", "Sin programación para ejecutar", 1, 1, 1)
                    End If
                End If
            ...
            Catch ex As Exception
                _resultado = New ResponseObject(New ErrorInfo(ex.Message, ex.StackTrace, 0, ex.Source))
                ReglasLog.ReglasLog.CrearLog("VerificaProgramacion", _resultado.ErrorInfo.Origen & ". " &
                                             _resultado.ErrorInfo.Mensaje & ". " & _resultado.ErrorInfo.Detalle,
                                             ReglasLog.ReglasLog.TipoLog.Err, 1, 1)
            ...
            ReglasLog.ReglasLog.CrearLog("VerificaProgramacion", "Fin", 1, 1, 1)
            ...
            Try
                ...
                ElseIf _resultado.Objeto = False Then
                    ReglasLog.ReglasLog.CrearLog(_proceso, Etiqueta & " | Validar Archivos Origen (no hay archivos para copiar)  | Fin",
                                                 ReglasLog.ReglasLog.TipoLog.Info, Programacion.VerbosidadLog, 1)
                    GoTo Fin
                ...
                If Not Programacion.RutaRespaldo Is Nothing And Programacion.RutaRespaldo <> "" Then
                    ...
                    If Not _rutaValida.IsError And _rutaValida.Objeto = True Then
                        ...
                        End If
                    Else
                        ReglasLog.ReglasLog.CrearLog(_proceso, Etiqueta & " | Respaldar | No existe información para respaldar.", 1, Programacion.VerbosidadLog, 1)
                    End If
...
Copiar:
                If Not _resultado.IsError Then
                    ...
                    ReglasLog.ReglasLog.CrearLog(_proceso, Etiqueta & " | Copiar | Inicio (intento " & _intentosCopia.ToString() & ")", 1, Programacion.VerbosidadLog, 1)
            ...
            Catch ex As Exception
                ReglasLog.ReglasLog.CrearLog(_proceso, Etiqueta & " CopiarArchivos | " & ex.Source & ". " & ex.Message & ". " & ex.StackTrace, 3, Programacion.VerbosidadLog, 1)
